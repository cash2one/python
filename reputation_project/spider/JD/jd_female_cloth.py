...
class Handler(BaseHandler):
    ...
    def step_first(self, response):
        ...
        for t in d('.gl-i-wrap.j-sku-item .p-name a').items():
            self.crawl(t.attr.href, callback=self.step_second, save={'name': t.text()}, retries=100)

    def step_second(self, response):
        txt = response.text
        sku_name = response.save.get('name')
        sku_id = pat_sku.findall(response.url)[0]
        # 针对一个SPU下多个SKU返回评论内容相同的情况，仅爬取其中一个SKU的评论信息
        if not sku_id in S:
            url = 'http://club.jd.com/productpage/p-' + sku_id + '-s-0-t-3-p-0.html'
            self.crawl(url, callback=self.my_result, retries=100, save={'sku': sku_id, 'name': sku_name})
    ...
    def step_third(self, response):
        ...
        if urls:
            for url_t in urls:
                self.crawl(url_t, callback=self.step_third, save=s, retries=100)
        return [
            sku_name,
            url,
            response.text,
