...
def get_hrefs():
    ...
    # print(len(href))
    # return href
    return ['http://mall.jd.com/shopLevel-121604.html']
    ...
    def getjudge(self):
        for url in self.url_list:
            try:
                ...
                if temp_1 == 'jIconLow':
                    judge_total_avg = '-' + soup.find(attrs={'class': 'jMallInfo'}).find_all('tr')[3].find_all('span')[
                        1].em.contents[0]
                else:
                    judge_total_avg = \
                        soup.find(attrs={'class': 'jMallInfo'}).find_all('tr')[3].find_all('span')[1].em.contents[0]
                ...
                for i in range(len(temp_2)):
                    ...
                    try:
                        name['judge_title_' + str(i) + '_avg'] = '-' + temp_2[i].div.find(
                    ...
                    for x in range(len(temp_3)):
                        ...
                        # except:name['judge_'+str(i)+'_'+str(x)+'_avg']=temp_3[x].find_all('span')[-1].em.string
                            ...
                            name['judge_' + str(i) + '_' + str(x) + '_avg'] = '-'
                result = [company_name, judge_total, judge_total_avg,
                          judge_title_0, judge_title_0_avg, judge_0_0, judge_0_0_avg, judge_0_1, judge_0_1_avg,
                          judge_0_2, judge_0_2_avg,
                          judge_title_1, judge_title_1_avg, judge_1_0, judge_1_0_avg, judge_1_1, judge_1_1_avg,
                          judge_1_2, judge_1_2_avg, judge_1_3, judge_1_3_avg,
                          judge_title_2, judge_title_2_avg, judge_2_0, judge_2_0_avg, judge_2_1, judge_2_1_avg,
                          judge_2_2, judge_2_2_avg, judge_2_3, judge_2_3_avg]
                queue.put(result)
                print(result)
            except:
                continue
...
if __name__ == '__main__':
    ...
    for i in range(thread_count):
        temp = getJudge(url_total[((len(url_total) + thread_count - 1) / thread_count) * i:((len(
            url_total) + thread_count - 1) / thread_count) * (i + 1)])
    ...
    writer = MyCsv.Write_Csv(path='d:/spider/jd', name='jd_companyname', result=res,
                             title=['company_name', 'judge_total', 'judge_total_avg',
                                    'judge_title_0', 'judge_title_0_avg', 'judge_0_0', 'judge_0_0_avg', 'judge_0_1',
                                    'judge_0_1_avg', 'judge_0_2', 'judge_0_2_avg',
                                    'judge_title_1', 'judge_title_1_avg', 'judge_1_0', 'judge_1_0_avg', 'judge_1_1',
                                    'judge_1_1_avg', 'judge_1_2', 'judge_1_2_avg', 'judge_1_3', 'judge_1_3_avg',
                                    'judge_title_2', 'judge_title_2_avg', 'judge_2_0', 'judge_2_0_avg', 'judge_2_1',
                                    'judge_2_1_avg', 'judge_2_2', 'judge_2_2_avg', 'judge_2_3', 'judge_2_3_avg'])
